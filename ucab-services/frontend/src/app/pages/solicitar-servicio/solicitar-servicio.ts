...
export class SolicitarServicio implements OnInit {
    ...
    this.servicioService.obtenerServicios().subscribe({
      next: (data) => {
        ...
        this.route.queryParams.subscribe((params) => {
          if (params['servicio']) {
            const servEncontrado = this.serviciosBD.find(
              (s) => s.codigoServicio === params['servicio'],
            );
            if (servEncontrado) {
  ...
  getImagenServicio(codigo: string): string {
    ...
    if (cod.includes('lab') || cod.includes('tec'))
      return 'https://images.unsplash.com/photo-1629904853716-f0bc54eea481?q=80&w=600&auto=format&fit=crop';
    if (cod.includes('dep') || cod.includes('can'))
      return 'https://images.unsplash.com/photo-1526676037777-05a232554f77?q=80&w=600&auto=format&fit=crop';
